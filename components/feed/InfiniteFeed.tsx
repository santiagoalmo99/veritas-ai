            ...
            {source === 'gdelt' ? (
              <>
                ...
                <span className="text-[10px] font-display font-black uppercase tracking-[0.2em] text-[var(--color-text-secondary)]">
                  {t.realTime} <span className="text-[var(--color-accent)]">GDELT 2.0 PROJECT</span>
                </span>
              </>
            ) : (
              <>
                <div className="flex h-2 w-2 rounded-full bg-[var(--color-text-tertiary)] opacity-50" />
                <span className="text-[10px] font-display font-black uppercase tracking-[0.2em] text-[var(--color-text-tertiary)]">
                  {t.demoData} {error ? `— ${error}` : ''}
                </span>
              </>
            )}
          </div>
