...
async function seed() {
    ...
    try {
        ...
        const { error: techError } = await supabase.from('techniques').upsert(mock_data_1.TECHNIQUES.map(t => ({
            id: t.id,
            slug: t.slug,
            name: t.name,
            name_es: t.nameEs,
        ...
        const { error: outletError } = await supabase.from('media_outlets').upsert(mock_data_1.MEDIA_OUTLETS.map(m => ({
            id: m.id,
            name: m.name,
            domain: m.domain,
        ...
        const { error: journoError } = await supabase.from('journalists').upsert(JOURNALISTS.map(j => ({
            id: j.id,
            name: j.name,
            score: j.score,
        ...
        for (const article of mock_data_1.MOCK_ARTICLES) {
            // Upsert Article
            ...
            if (article.techniquesDetected?.length > 0) {
                ...
                const { error: techLinkError } = await supabase.from('article_techniques').insert(article.techniquesDetected.map(td => ({
            ...
            if (article.analysisLogs?.length > 0) {
                ...
                const { error: logError } = await supabase.from('article_analysis_logs').insert(article.analysisLogs.map(log => ({
                    article_id: article.id,
                    step_id: log.stepId,
                    status: log.status,
